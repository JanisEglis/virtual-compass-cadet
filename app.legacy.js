...
if (window.__CADET_APP_BOOTED__ === BUILD) {
  ...
  function wgsToLKS(lat, lng) {
    // ievade: WGS84 lat,lng
  ...
  onDomReady(() => {
    ...
    imgs.forEach(imgEl => {
      if (imgEl.complete) {
        ...
        imgEl.addEventListener('load', onOne, {
          once: true
        });
        imgEl.addEventListener('error', onOne, {
          once: true
        });
      }
    });
          ...
          recs.forEach(r => {
            r.addedNodes.forEach(n => {
              if (n && n.tagName === 'IMG' && n.classList.contains('leaflet-tile')) {
                n.addEventListener('load', onTile, {
                  once: true
                });
                n.addEventListener('error', onTile, {
                  once: true
                });
              }
            });
    ...
    const domReady = new Promise(res => {
      if (document.readyState === 'interactive' || document.readyState === 'complete') res();else document.addEventListener('DOMContentLoaded', res, {
        once: true
      });
    });
    ...
    window.addEventListener('error', () => finish('window-error'), {
      once: true
    });
    window.addEventListener('unhandledrejection', () => finish('unhandledrejection'), {
      once: true
    });
    ...
    function finish(reason) {
      ...
      render(100);
      console.debug('[preloader] finish:', reason, {
        done,
        total
      });
  ...
  if (_lck) _lck.addEventListener('click', function () {
  ...
  function hasClass(el, c) {
    if (!el) return false;
    return 'classList' in el ? el.classList.contains(c) : new RegExp('(^|\\s)' + c + '(\\s|$)').test(el.className);
  }
  ...
  function removeClass(el, c) {
    ...
    if ('classList' in el) {
      ...
      el.className = el.className.replace(new RegExp('(^|\\s)' + c + '(\\s|$)', 'g'), ' ').replace(/\s+/g, ' ').trim();
    }
  }
  function setBtnText(btn, txt) {
    if (!btn) return;
    if ('textContent' in btn) btn.textContent = txt;else btn.innerText = txt;
  }
  ...
  function closeBothSelectorsLegacy() {
    ...
    if (leftPanel) {
      addClass(leftPanel, 'hidden-left');
    }
    if (rightPanel) {
      addClass(rightPanel, 'hidden');
    }
  ...
  function showBothSelectorsOnce() {
    ...
    var rightPanel = qs(RIGHT_PANEL_SEL);
    if (leftPanel) {
      removeClass(leftPanel, 'hidden-left');
      removeClass(leftPanel, 'hidden');
    }
    if (rightPanel) {
      removeClass(rightPanel, 'hidden');
    }
    if (window.__updateMapSafeAreas) window.__updateMapSafeAreas();
  }
  ...
  function armSelectorIdleCloseLegacy(panel, delayMs) {
    ...
    var evs = ['pointerdown', 'pointermove',
    // moderni
    ...
    for (var i = 0; i < evs.length; i++) {
      try {
        panel.addEventListener(evs[i], handler, true);
      } catch (e) {}
    }
    ...
    function cleanup() {
      ...
      for (var i = 0; i < evs.length; i++) {
        try {
          panel.removeEventListener(evs[i], handler, true);
        } catch (e) {}
      }
  ...
  function bindAutoCloseOnToggleLegacy() {
    ...
    if (rightBtn && rightPanel) {
      rightBtn.addEventListener('click', function () {
        ...
        if (!hasClass(rightPanel, 'hidden')) armSelectorIdleCloseLegacy(rightPanel, 5000);else if (rightPanel._idleCleanup) rightPanel._idleCleanup();
      }, false);
    }
    if (leftBtn && leftPanel) {
      leftBtn.addEventListener('click', function () {
        if (!hasClass(leftPanel, 'hidden-left')) armSelectorIdleCloseLegacy(leftPanel, 5000);else if (leftPanel._idleCleanup) leftPanel._idleCleanup();
      }, false);
    }
  }
  ...
  (function () {
    ...
    function tryDispatch(msg) {
      ...
      if (d && typeof d.event === 'function') {
        d.event(msg.channel, msg.type, msg.data);
        return true;
      }
      // 2) Alternatīva globāla funkcija
      if (typeof window.__devlog === 'function') {
        window.__devlog(msg);
        return true;
      }
      return false;
  ...
  (function () {
    ...
    (function () {
      const topSelectors = ['#fullscreenMessage:not(.fs-message-hidden)', '.top-bar', '.dropdown-menu.visible', '#contentFrame.active', '#instructionFrame.active'
      // ⬅️ NOŅEMAM .position-selector un .position-selector-left,
      // lai sānu paneļi neietekmētu top drošo zonu
      ];
      const bottomSelectors = ['#about', '#iframeContainerAbout', '#iframeContainerQR'];
      ...
      function getTopSafePx() {
        ...
        topSelectors.forEach(sel => {
          document.querySelectorAll(sel).forEach(el => {
            px = Math.max(px, visibleOverlapTop(el));
          });
        });
      ...
      function getBottomSafePx() {
        ...
        bottomSelectors.forEach(sel => {
          document.querySelectorAll(sel).forEach(el => {
            px = Math.max(px, visibleOverlapBottom(el));
          });
        });
        return Math.round(px);
      }
...
#onlineMap .leaflet-popup   { z-index: 600; }
    ...
    function createSafeTileLayer(url, opts = {}) {
      var _opts$maxNativeZoom;
      const defaults = {
        maxZoom: 20,
        maxNativeZoom: (_opts$maxNativeZoom = opts.maxNativeZoom) !== null && _opts$maxNativeZoom !== void 0 ? _opts$maxNativeZoom : 19,
        subdomains: 'abc',
        updateWhenIdle: true,
        ...
        keepBuffer: 3,
        detectRetina: false,
        // nepasūta 2× vairāk flīzes
        noWrap: true,
        // nerauj ārpus pasaules robežām
        crossOrigin: true,
      ...
      layer.on('tileerror', e => {
        try {
          e.tile.src = defaults.errorTileUrl;
        } catch (_) {}
      });
      return layer;
    }
    function installTileErrorWatch(layer, opts) {
      const name = (opts === null || opts === void 0 ? void 0 : opts.name) || 'layer';
      const threshold = (opts === null || opts === void 0 ? void 0 : opts.threshold) || 12;
      const windowMs = (opts === null || opts === void 0 ? void 0 : opts.windowMs) || 3000;
      const onTrip = (opts === null || opts === void 0 ? void 0 : opts.onTrip) || (() => {});
      let errs = [];
      function purge() {
        const t = Date.now();
        errs = errs.filter(x => t - x < windowMs);
      }
      function onErr() {
        errs.push(Date.now());
        purge();
    ...
    function initMap() {
      ...
      if (!window.L) {
        console.warn('Leaflet nav ielādēts');
        return false;
      }
      ...
      map = L.map(mapDiv, {
        ...
        // ADD
        maxZoom: 20,
        // ADD (varēsi iezūmot dziļāk par native)
      ...
      topo.on('load', () => {
        topoErrors = 0;
      }); // ja ielādējas, skaitītāju nullējam
      topo.on('tileerror', () => {
      ...
      [osm, topo, esri, hot, cyclo, osmDe, osmFr, cartoLight, lvmTopo50_wms, lvmOSM, hiking, cycling, rail, seamarks].forEach(l => l.on('tileerror', e => {
        // nerādīt “salūzušo bildi” + logā redzēt avotu
        try {
          if (e && e.tile) e.tile.src = 'data:image/gif;base64,R0lGODlhAQABAAAAACw=';
        } catch (_) {}
        console.warn('[tileerror]', l && l._url, (e === null || e === void 0 ? void 0 : e.coords) || e);
      }));
      ...
      scalePickCtl.onAdd = function () {
        ...
        select.addEventListener('change', () => {
          ...
          map.setZoom(zoomForScale(targetScale), {
            animate: true
          });
    ...
    `;
      ...
      <div class="box">
        ...
        window.__setPrintProgress = (loaded, total) => {
          ...
          if (bar) bar.style.width = pct + '%';
          if (sub) sub.textContent = total > 0 ? `Ielādētas flīzes: ${loaded}/${total} (${pct}%)` : `Ielādētas flīzes: ${loaded} (${pct}%)`;
        };
      }
      ...
      function waitForMapToRender(map, opts = {}) {
        ...
        return new Promise(resolve => {
          ...
          let rafId = null,
            toId = null;
          const isMapImg = n => n && n.tagName === 'IMG' && (n.classList.contains('leaflet-tile') || n.classList.contains('leaflet-image-layer'));
            ...
            muts.forEach(m => {
              m.addedNodes && m.addedNodes.forEach(n => {
                ...
                if (n && n.querySelectorAll) {
                  n.querySelectorAll('img.leaflet-tile, img.leaflet-image-layer').forEach(img => {
                    arm(img);
                    added = true;
                  });
                }
          ...
          if (map && map.on) {
            ...
            cleanup = function () {
              try {
                map.off('move', kick).off('moveend', kick).off('zoom', kick).off('zoomend', kick).off('layeradd', kick).off('layerremove', kick);
              } catch (_) {}
              _cleanup();
            };
          }
      ...
      function __centerPxInContainerFromOverlayOrViewport(containerRect) {
        ...
        if (scr) {
          return {
            x: scr.x - containerRect.left,
            y: scr.y - containerRect.top
          };
        }
      ...
      async function __recenterMapToLL(map, ll) {
        ...
        let sz = map.getSize();
        map.panBy([sz.x / 2 - pt.x, sz.y / 2 - pt.y], {
          animate: false
        });
      ...
      async function prepareMapForPrintLgIa(opts) {
        ...
        map.setZoom(zoomForScale(scale), {
          animate: false
        });
        ...
        const prevInlineStyle = (mapEl === null || mapEl === void 0 ? void 0 : mapEl.getAttribute('style')) || '';
        if (mapEl) {
        ...
        await new Promise(r => requestAnimationFrame(r));
        map.invalidateSize(true);
        map.setView(keepCenter, map.getZoom(), {
          animate: false
        });
        ...
        setTimeout(async () => {
          ...
          try {
            window.closeBothSelectorsLegacy && window.closeBothSelectorsLegacy();
          } catch (_) {}
          try {
        ...
        @media print{
          ...
          map.invalidateSize(true);
          if (map._resetView) map._resetView(keepCenter, map.getZoom(), true);else map.setView(keepCenter, map.getZoom(), {
            animate: false
          });
          {
            ...
            map.panBy([sz.x * 0.5 - onMap.x, sz.y * 0.5 - onMap.y], {
              animate: false
            });
          }
          ...
          await new Promise(r => requestAnimationFrame(r));
          {
            ...
            map.panBy([sz2.x * 0.5 - p2.x, sz2.y * 0.5 - p2.y], {
              animate: false
            });
          }
          ...
          function cleanup() {
            ...
            try {
              ...
              if (dimEl) {
                if (prevDimStyle !== null) dimEl.setAttribute('style', prevDimStyle);else dimEl.removeAttribute('style');
              }
            } catch (_) {}
            ...
            try {
              map.invalidateSize(true);
              map.setView(prevView.center, prevView.zoom, {
                animate: false
              });
            } catch (_) {}
      ...
      function injectDynamicPrintStyle(fmt, orient) {
        ...
        const base = fmt === 'A3' ? orient === 'portrait' ? {
          w: 277,
          h: 400
        } : {
          w: 400,
...
body.print-mode #printNorthTR .n{
  ...
  `;
      ...
      function buildPrintFooterLgIa(scaleVal, title) {
        ...
        document.body.appendChild(scaleTop);
        els.push(scaleTop);
    ...
    `;
        ...
        function addLine(points, isMajor, putLabel, labelLatLng, labelText) {
          ...
          L.polyline(points, {
            ...
            color: '#ffffff',
            opacity: thin ? .9 : .92,
            weight: isMajor ? majorOut : minorOut,
            lineJoin: 'round',
            lineCap: 'round'
          }).addTo(gLines);
          L.polyline(points, {
      ...
      function createLKSGridLayers() {
        const lineStyle = {
          ...
          weight: 2.6,
          opacity: 0.95
        }; // melnas, izteiktas līnijas

        const labelStyle = {
          className: 'lks-grid-label'
        }; // CSS vari piekrāsot, ja vajag
      ...
      function currentPrintScale() {
        // “1:xxxx” pie 0.28 mm/pix
      ...
      const LgiaScale = L.Control.extend({
        ...
        onAdd: function () {
          ...
          Object.assign(bar.style, {
            ...
            // apgriež joslu otrādi (stabiņi uz leju)
        ...
        onRemove: function () {
          map.off('move zoom zoomend', this._update);
        },
        _update: function () {
          ...
          if (scale >= 4500 && scale <= 5500) {
            ...
            for (let s of steps) {
              if (s <= maxMeters) targetMeters = s;else break;
            }
            if (maxMeters < steps[0]) targetMeters = steps[0];
          }
          const px = targetMeters / mpp;
          this._els.bar.style.width = px + 'px';
          this._els.label.textContent = targetMeters >= 1000 ? (targetMeters % 1000 ? (targetMeters / 1000).toFixed(1) : targetMeters / 1000) + ' km' : targetMeters + ' m';
        }
      });
      ...
      map.on('moveend zoomend', updateRatio);
      updateRatio();

      // apakšējais kreisais info (Lat/Lng + MGRS) + klikšķis — kopēt
      const posCtl = L.control({
        position: 'bottomleft'
      });
      posCtl.onAdd = function () {
        ...
        div.addEventListener('click', async () => {
          ...
          if (ok) {
            ...
            setTimeout(() => {
              div.style.background = oldBG || 'rgba(0,0,0,.5)';
            }, 1200);
          } else {
      ...
      function makeLayersClickOnly(layersCtl) {
        ...
        function bindToggle() {
          ...
          ['click', 'touchstart', 'pointerdown', 'mousedown'].forEach(ev => {
            try {
              link.removeEventListener(ev, onToggle, false);
            } catch (_) {}
          });
          if (supportsPointer) {
            link.addEventListener('pointerdown', onToggle, {
              passive: false
            });
          } else if ('ontouchstart' in window) {
            link.addEventListener('touchstart', onToggle, {
              passive: false
            });
            link.addEventListener('mousedown', onToggle, {
              passive: false
            }); // rezerve
          } else {
            link.addEventListener('mousedown', onToggle, {
        ...
        bindToggle();
        new MutationObserver(bindToggle).observe(c, {
          childList: true,
          subtree: true
        });
        ...
        wireInputs();
        new MutationObserver(wireInputs).observe(c, {
          childList: true,
          subtree: true
        });
      ...
      function rowsForPopup(lat, lng) {
        ...
        if (lksOn) {
          ...
          rows.push({
            id: 'lks',
            label: 'LKS-92',
            value: `E ${Math.round(L.E)}, N ${Math.round(L.N)}`
          });
        }
        return rows;
      }
  ...
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
    ...
    <div class="coord-popup">
      ...
      map.on('popupopen', ev => {
        ...
        root.querySelectorAll('button.copy-btn[id^="copy-"]').forEach(btn => {
          ...
          const val = ((_root$querySelector = root.querySelector(`#${id}Val`)) === null || _root$querySelector === void 0 ? void 0 : _root$querySelector.textContent) || '';
    ...
    onDomReady(() => {
      ...
      if (localStorage.getItem('onlineMapActive') === '1') {
        leafletReady.then(() => showOnlineMap()).catch(() => localStorage.setItem('onlineMapActive', '0'));
      }
  ...
  if (uploadBtn) {
    uploadBtn.addEventListener('click', () => {
      openChooserModal().then(choice => {
        if (!choice) return;
        if (choice.kind === 'file') loadFromFile(choice.file);else if (choice.kind === 'url') loadFromURL(choice.url);
      });
    });
  }
 ...
 <span class="ico" aria-hidden="true">
  ...
  onDomReady(() => {
    ...
    (function initCompassSafe() {
      const start = () => {
        const ok = document.getElementById('compassContainer') && document.getElementById('compassInner') && document.getElementById('compassScaleContainer') && document.getElementById('compassScaleInner');
        if (!ok) {
        ...
        window.addEventListener('load', updateCompassTransform, {
          once: true
        });
    ...
    (function bindCompassSection() {
      function bindCompassListeners() {
        ...
        if (!cc) {
          requestAnimationFrame(bindCompassListeners);
          return;
        } // gaida, līdz elements parādās
      ...
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bindCompassListeners, {
          once: true
        });
      } else {
        bindCompassListeners();
      }
      window.addEventListener('load', bindCompassListeners, {
        once: true
      });
  ...
  (function compassWatchdog() {
    ...
    function tick() {
      ...
      const init = window.COMPASS_INIT || {
        left: 0,
        top: 0
      };
      const ok = Math.abs(left - init.left) < 1 && Math.abs(top - init.top) < 1;
      if (!ok && t < MAX_MS) {
        id = setTimeout(tick, STEP);
        t += STEP;
      }
  ...
  document.addEventListener("click", function (event) {
    ...
    if (instructionMenu && instructionButton && !instructionMenu.contains(event.target) && !instructionButton.contains(event.target)) {
      instructionMenu.classList.remove("visible");
      instructionButton.classList.remove("active");
    }
    if (materialsMenu && materialsButton && !materialsMenu.contains(event.target) && !materialsButton.contains(event.target)) {
  ...
  (function () {
    ...
    btns.reverse().forEach(function (b) {
      shell.appendChild(b);
    });
    ...
    dockCap.className = 'dock-cap';
    dockCap.setAttribute('aria-hidden', 'true');
    ...
    dimWrap.innerHTML = '<input id="mapDimmerRange" type="range" min="0" max="80" step="1">' + '<span class="value" id="mapDimmerValue"></span>';
    ...
    setTimeout(function () {
      ...
      apply();
      new MutationObserver(apply).observe(bc, {
        attributes: true,
    ...
    function updateCapGeometry() {
      ...
      var rightCP1x = REx + (w - REx) * 0.18,
        rightCP1y = sideMidY;
      var rightCP2x = w,
        rightCP2y = r + (h - r) * 0.82;
      var leftCP2x = 0,
        leftCP2y = r + (h - r) * 0.82;
      var leftCP1x = LEx - (LEx - 0) * 0.18,
        leftCP1y = sideMidY;
      dockCap.setAttribute('viewBox', '0 0 ' + w + ' ' + h);
      dockCap.setAttribute('width', w);
      dockCap.setAttribute('height', h);
      dockCap.innerHTML = '<defs>' + '<linearGradient id="capFill" x1="0" y1="0" x2="0" y2="1">' + '<stop offset="0%"  stop-color="rgba(40,44,52,.96)"/>' + '<stop offset="100%" stop-color="rgba(30,33,40,.96)"/>' + '</linearGradient>' + '<radialGradient id="capShine" cx="50%" cy="-30%" r="120%">' + '<stop offset="0%" stop-color="rgba(255,255,255,.18)"/>' + '<stop offset="60%" stop-color="rgba(255,255,255,0)"/>' + '</radialGradient>' + '</defs>' + '<path d="' + 'M ' + TLx + ',0 H ' + TRx + ' A ' + r + ',' + r + ' 0 0 1 ' + REx + ',' + REy + ' ' + 'C ' + rightCP1x + ',' + rightCP1y + ' ' + rightCP2x + ',' + rightCP2y + ' ' + w + ',' + h + ' ' + 'L 0,' + h + ' C ' + leftCP2x + ',' + leftCP2y + ' ' + leftCP1x + ',' + leftCP1y + ' ' + LEx + ',' + LEy + ' ' + 'A ' + r + ',' + r + ' 0 0 1 ' + TLx + ',0 Z" fill="url(#capFill)" stroke="rgba(210,34,34,.38)" stroke-width="1" />' + '<path d="' + 'M ' + TLx + ',0 H ' + TRx + ' A ' + r + ',' + r + ' 0 0 1 ' + REx + ',' + REy + ' ' + 'C ' + rightCP1x + ',' + rightCP1y + ' ' + rightCP2x + ',' + rightCP2y + ' ' + w + ',' + h + ' ' + 'L 0,' + h + ' C ' + leftCP2x + ',' + leftCP2y + ' ' + leftCP1x + ',' + leftCP1y + ' ' + LEx + ',' + LEy + ' ' + 'A ' + r + ',' + r + ' 0 0 1 ' + TLx + ',0 Z" fill="url(#capShine)" />';
    }
    ...
    function setTipX(px) {
      ...
      function step() {
        ...
        if (Math.abs(targetX - currentX) > 0.5) {
          ...
          shell.style.setProperty('--tipX', targetX + 'px');
          raf = null;
        }
      }
      raf = requestAnimationFrame(step);
    ...
    function setTipY(py) {
      shell.style.setProperty('--tipY', py + 'px');
    }
    function isVertical() {
    ...
    function hideTip() {
      shell.classList.remove('show-label', 'show-cap');
    }
  ...
  (function keepDockMarginsFromCSS() {
    ...
    strip();
    new MutationObserver(strip).observe(shell, {
      subtree: true,
      attributes: true,
  ...
  (function () {
    function fitDock() {
      ...
      let scale = isVertical ? Math.min(1, maxH / natural.height) : Math.min(1, maxW / natural.width);
  ...
  (function () {
    ...
    function updateDimmerSpan() {
      ...
      var rows = Math.max(1, children.filter(function (el) {
...
`;
    ...
    function dockButtons() {
      bc.classList.add('docked');
      window.__updateMapSafeAreas && window.__updateMapSafeAreas();
    }
    function showButtons() {
      bc.classList.remove('docked');
      window.__fitDock && window.__fitDock();
      window.__updateMapSafeAreas && window.__updateMapSafeAreas();
    }
    ...
    const leftSel = document.getElementById('positionSelectLeft');
    const rightSel = document.getElementById('positionSelect');
    function refreshBySelect() {
      handle.innerHTML = GRID_ICON;
      window.__fitDock && window.__fitDock();
    }
    ...
    ['pointerdown', 'mousedown', 'touchstart'].forEach(ev => {
      map && map.addEventListener(ev, dockButtons, {
        passive: true
      });
      compass && compass.addEventListener(ev, dockButtons, {
        passive: true
      });
    });
...
#printAreaOverlay .label{position:absolute;right:8px;bottom:8px;font:600 12px/1.2 system-ui,sans-serif;padding:6px 8px;background:rgba(0,0,0,.55);color:#fff;border-radius:6px}`;
    ...
    function showOverlay(ms = 5000) {
      ...
      el.querySelector('.label').textContent = `print-media: ${w}×${h} mm  ≈  ${Math.round(wpx)}×${Math.round(hpx)} px`;
  ...
  (_matchMedia => {
    if (window.__VC_TOUR_V11__) return;
    window.__VC_TOUR_V11__ = 1;

    // ——— Utils
    const qs = (s, r = document) => r.querySelector(s);
    const vis = el => !!el && el.offsetWidth > 0 && el.offsetHeight > 0 && getComputedStyle(el).visibility !== 'hidden' && getComputedStyle(el).display !== 'none';
    const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
    const VW = () => innerWidth;
    const VH = () => {
      var _visualViewport;
      return ((_visualViewport = visualViewport) === null || _visualViewport === void 0 ? void 0 : _visualViewport.height) || innerHeight;
    };
    const isTouch = 'ontouchstart' in window || ((_matchMedia = matchMedia) === null || _matchMedia === void 0 ? void 0 : _matchMedia('(pointer: coarse)').matches);
    ...
    `;
      sh.appendChild(Object.assign(document.createElement('style'), {
        textContent: css
      }));
      overlay = Object.assign(document.createElement('div'), {
        className: 'overlay'
      });
      ring = Object.assign(document.createElement('div'), {
        className: 'ring'
      });
      tip = Object.assign(document.createElement('div'), {
        className: 'tip'
      });
      progress = Object.assign(document.createElement('div'), {
        className: 'progress'
      });
    ...
    function showUI() {
      ...
      [overlay, ring, tip, progress].forEach(n => n.classList.remove('hide'));
    }
    function hideUI() {
      host.style.pointerEvents = 'none';
      [overlay, ring, tip, progress, hint].forEach(n => n.classList.add('hide'));
    }
    ...
    function placeTip(rect, key, pref = 'right') {
      ...
      const gap = 12,
        tw = Math.min(tip.offsetWidth || 360, VW() - 16),
      ...
      const p = clampPos(x, y, tw, th);
      tip.style.left = p.x + 'px';
      tip.style.top = p.y + 'px';
      posMemo[key] = pos;
    }
    function resetTipMemo() {
      for (const k in posMemo) delete posMemo[k];
    }
    ...
    function demoRotate(el) {
      if (!el) return () => {};
      const prev = el.style.transform;
      let k = 0;
      const id = setInterval(() => {
        k++;
        el.style.transform = `${prev} rotate(${Math.sin(k / 4) * 4}deg)`;
      }, 60);
      return () => {
        clearInterval(id);
    ...
    function demoSlider(input) {
      if (!input) return () => {};
      const orig = input.value;
      let t = 0;
      const id = setInterval(() => {
        t += 0.06;
        const v = Math.round(50 + 45 * Math.sin(t));
        input.value = v;
    ...
    const T = {
      compass: isTouch ? 'Skārienā: velc pārvieto; pincete — mērogs; 2 pirkstu grieziens — griež izvēlēto daļu.' : 'Pele: velc pārvieto; ALT+rullītis — mērogs; SHIFT — griež bāzi, CTRL — griež skalu.',
    ...
    // pogu novietojuma selektori	  
    {
      ...
      sel: '#positionSelect',
      title: 'Izvēlies: Apakša / Pa kreisi / Pa labi',
      place: 'left',
      optional: true
    }, {
    ...
    // Kompass — vispārīgi
    {
      ...
      title: 'Kompass (pārvieto/mērogo/griez)',
      body: T.compass,
      place: 'right'
    },
    // Režīma pogas
    {
      ...
      title: 'Bāze ⇄ Skala',
      body: 'Pārslēdz, kuru daļu grozīt ar žestiem.',
      place: 'top',
      optional: true
    }, {
      sel: '#lockRotationMode',
      title: 'Bloķēt rotāciju',
      body: 'Fiksē rotāciju ērtai tālummaiņai.',
      place: 'top',
      optional: true
    }, {
      sel: '#rotateCompass90',
      title: 'Koordināšu noteikšanas opcijas',
      body: 'Izvēlies ar kādu metodi noteiksi koordinātes.',
      place: 'top',
      optional: true
    }, {
      sel: '#resetCompass',
      title: 'Atjauno kompasu',
      body: 'Atgriež sākumstāvoklī (pozīcija un izmērs).',
      place: 'top',
      optional: true
    },
    // Lokālā karte
    {
      ...
      body: 'Ielādē JPG/PNG/PDF (vienu vai vairāku lapu fails)/URL kartes.',
      place: 'top',
      optional: true
    }, {
      sel: '#mapCanvas',
      title: 'Lokālā karte',
      body: isTouch ? 'Tālummaiņa ritinot pelītes rulīti vai ar diviem pikstiem velkot uz augšu/leju touchpad; Pārvieto spiezot peles kreiso pogu un velkot.' : 'Tālummaiņa - divu pirkstu tuvināšana/tālināšana; Pārvietot - peskaries un velc.',
      place: 'bottom',
      optional: true
    }, {
      sel: '#resizeHandle',
      title: 'Izmēra rokturis',
      body: 'Uzspied un velc, lai mainītu lokālās kartes izmēru (ieteicams kalibrēšanai ar kompasu).',
      place: 'bottom',
      optional: true
    }, {
      sel: '#resetMap',
      title: 'Atjauno lokālo karti.',
      body: 'Atgriežas sākumstāvoklī (pozīcija un izmērs).',
      place: 'top',
      optional: true
    },
    // Tiešsaistes karte + slāņi + PDF
    {
      ...
      title: 'Tiešsaistes karte',
        ...
        var _qs;
        const m = qs('#onlineMap');
        if (!m || !vis(m)) (_qs = qs('#toggleOnlineMap')) === null || _qs === void 0 || _qs.click();
      },
      body: 'Ritenis/touchpad/žesti — tālummaiņa; nospied un velc — pārvieto.',
      place: 'bottom',
      optional: true
    }, {
      sel: '.leaflet-control-zoom-in',
      title: 'Tālummaiņas pogas',
      body: 'Tiešsaistes kartes + / −',
      place: 'left',
      optional: true
    }, {
      sel: '.leaflet-control-layers-toggle',
      title: 'Slāņi',
      ensure: () => {
        var _qs2;
        return qs('.leaflet-control-layers') || ((_qs2 = qs('.leaflet-control-layers-toggle')) === null || _qs2 === void 0 ? void 0 : _qs2.click());
      },
      body: 'Pamatkartes un pārklājumi.',
      place: 'left',
      optional: true
    }, {
      sel: '#mapDimmerRange',
      title: 'Tumšuma slīdnis',
      body: 'Maini lokālās un tiešsaistes kartes spilgtumu, lai izceltu kompasu. Automātiski tiek noņemts drukā',
      place: 'right',
      optional: true
    }, {
      sel: '#preparePrintBtn',
      title: 'Sagatavot drukai',
      body: 'Izvēlies formātu/mērogu, sagatavo drukai vai saglabāšanai PDF.',
      place: 'left',
      optional: true
    },
    // Pilnekrāns
    {
      ...
      title: 'Pilnekrāns',
      body: 'Ieslēgt/izslēgt ērtākam darbam.',
      place: 'top',
      optional: true
    },
    // Info/Par
    {
      ...
      title: 'Detalizētas instrukcijas',
      place: 'bottom',
      optional: true
    }, {
      sel: '#toggleMaterials',
      ...
      sel: '#about',
      title: 'Koplietošanas QR auditorijai / Par rīku / Ziņot.',
      place: 'top',
      optional: true
    }];
      ...
      (function wait() {
        ...
        tip.querySelectorAll('[data-act]').forEach(b => {
          b.onclick = ev => {
            ...
            if (a === 'prev') prev();else if (a === 'next') idx < STEPS.length - 1 ? next() : stop();else if (a === 'close') stop();
          };
        });
      })();
    }
    ...
    new MutationObserver(() => running && reposition()).observe(document.documentElement, {
      subtree: true,
      childList: true,
      attributes: true,
    ...
    window.helpTour = {
      ...
      prev,
      go: n => {
        idx = clamp(n | 0, 0, STEPS.length - 1);
        run();
      }
    };
    ...
    function ensureFab() {
      ...
      if (!fab) {
        ...
        Object.assign(fab.style, {
          ...
          background: 'linear-gradient(180deg,#1b1f25,#371017aa)',
          color: '#fff',
